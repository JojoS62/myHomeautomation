...
//****************************************************************************************************************
#define NODEID         50                   //unique for each node on same network 40 Motion 50 Doorbell
...
ACMP		aCmp(ACMP::ACMP_I1, ACMP::BANDGAP, ACMP::NONE);
...
int main()
{
	...
	const char* msgLowPowerSensor	= "{\"Doorbell\":{\"status\":\"lowPowerSensor\"}}";
	const char* msgLowPowerExt		= "{\"Doorbell\":{\"status\":\"lowPowerExt\"}}";
	...
	if (WakeUp::wokeUpFromDeepSleep())					// triggered by PIR signal
	{
...

		{
			...
			{
				wait_ms(100);
				rfm.send(1, msgLowPowerSensor, strlen(msgLowPowerSensor));
			}
		}

		{
			// check external battery
			aCmp.set(ACMP::ACMP_I1, ACMP::BANDGAP, ACMP::NONE);
			wait_ms(1);
			bool lowPower = !aCmp.read();
			if (lowPower)
			{
				wait_ms(100);
				rfm.send(1, msgLowPowerExt, strlen(msgLowPowerExt));
			}
		}
	}
